...
const StagingPage = () => {
    ...
    useEffect(() => {
        ...
        useEffect(() => {
            if (!isLocationCodePromptOpen || isLocationCodeBypassReady) {
                return;
            }
        ...
        }, [isLocationCodePromptOpen, isLocationCodeBypassReady]);
    
        useEffect(() => {
            if (!isLocationCodeScannerOpen || !locationCodeScannerVideoRef.current) {
                return;
            }
    
            const detector = locationCodeScannerDetectorRef.current;
            const stream = locationCodeScannerStreamRef.current;
            const videoElement = locationCodeScannerVideoRef.current;
            ...
            const scan = async () => {
                try {
                    if (locationCodeScannerHandlingRef.current) {
                        locationCodeScannerFrameRef.current = window.requestAnimationFrame(scan);
                        return;
                    }
    
                    if (!videoElement.readyState || videoElement.readyState !== videoElement.HAVE_ENOUGH_DATA) {
                        locationCodeScannerFrameRef.current = window.requestAnimationFrame(scan);
                        return;
                    }
                    ...
                    if (Array.isArray(detections) && detections.length > 0) {
                        ...
                        const scannedCode = String(detection?.rawValue || '').trim();
                        setLocationCodeEntry(scannedCode);
                    }
                } catch (error) {
                    console.debug('Location code scanner detection error (expected in some browsers):', error);
                } finally {
                    locationCodeScannerHandlingRef.current = false;
                    locationCodeScannerFrameRef.current = window.requestAnimationFrame(scan);
                }
